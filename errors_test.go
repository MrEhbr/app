...
func TestOpError(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: &Error{Op: "test", Err: errors.New("test")},
		},
		{
			name: "err is an Error && err.Op is not empty",
			args: args{
				op:  "test",
				err: &Error{Op: "foo", Err: errors.New("test")},
			},
			want: &Error{Op: "test", Err: &Error{Op: "foo", Err: errors.New("test")}},
		},
		{
			name: "err is an Error && err.Op is empty",
			args: args{
				op:  "test",
				err: &Error{Err: errors.New("test")},
			},
			want: &Error{Op: "test", Err: errors.New("test")},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := OpError(tt.args.op, tt.args.err)
			if diff := cmp.Diff(tt.want, got, cmpopts.EquateErrors()); diff != "" {
				t.Errorf("OpError() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
